...
class usbcomm:
    ...
    def get_deflection(self):
        try:
            ret = self.dev.ctrl_transfer(0xC0, self.SEND_DEF, 0, 0, 2)
        except usb.core.USBError:
            print "Could not send SEND_VEL vendor request."
        else:
            return ret 
    def get_speed(self):
        try:
    ...
    def set_scale(self,scale):
        try:
            ret = self.dev.ctrl_transfer(0xC0, self.SET_SCALE, scale,0,0)
        except usb.core.USBError:
            print "Could not send set_scale vendor request."
    def get_defsign(self):
        try:
            ret = self.dev.ctrl_transfer(0xC0, self.SEND_DEFS, 0, 0, 1)
        except usb.core.USBError:
            print "Could not send SEND_MTRDIR vendor request."
        else:
            return ret
